...
class ApplicableFndRequest(BaseModel):
    vnd_filename: Optional[str] = ""
    vnd_name: Optional[str] = ""
    stage1: dict
...
@app.post("/api/vnd/applicable-fnd")
async def vnd_applicable_fnd(request: ApplicableFndRequest):
    """Пересчитать перечень применимых ФНД по параметрам этапа 1."""
    try:
        from pre_analysis import detect_applicable_fnd, normalize_stage1_answers
        ...
        stage1 = normalize_stage1_answers(request.stage1 or {})
        vnd_name = (request.vnd_name or "").strip() or os.path.basename(filename) or "ВНД"
        applicable_fnd = detect_applicable_fnd(
            stage1,
            os.path.basename(filename) if filename else vnd_name,
            vnd_name,
            vnd_text,
        )
        return {"applicable_fnd": applicable_fnd, "stage1": {**stage1, "applicable_fnd": applicable_fnd}}
    except HTTPException:
        raise
    except Exception as e:
        import traceback
...
async def vnd_analyze(request: AnalyzeVndRequest):
    ...
    try:
        ...
        from pre_analysis import detect_applicable_fnd

        pre_analysis["applicable_fnd"] = detect_applicable_fnd(
            pre_analysis,
            os.path.basename(filename) if filename else vnd_name,
            vnd_name,
            vnd_text,
        ...
        return {
            "response": content,
            "stage1": pre_analysis,
            "applicable_fnd": pre_analysis.get("applicable_fnd") or [],
            "vnd_name": vnd_name,
            "phase": "analysis",
