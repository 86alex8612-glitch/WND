...
DEFAULT_WORK_FOLDER = Path(r"C:\WND")
...
def read_work_folder() -> str:
    if not CONFIG_FILE.is_file():
        write_work_folder(str(DEFAULT_WORK_FOLDER))
        return str(DEFAULT_WORK_FOLDER.resolve())
...
def browse_folder_dialog(initial_dir: str = "") -> Optional[str]:
    """Нативный диалог выбора папки (Windows). Возвращает None при отмене."""
    try:
