...
Названия реквизитов (ИНН, ОГРН, ФИО и т.д.) сохраняются; значения заменяются на * по длине.
"""
...
_LABELED_VALUE_PATTERNS: List[Tuple[str, int]] = [
    ...
    (
        r"(\bИНН(?:\s*/\s*КПП)?\s*(?:№\.?|:)?\s*)"
        rf"({_DIGIT_VALUE}{{10,12}}(?:\s*/\s*{_DIGIT_VALUE}{{8,9}})?)",
        2,
    ),
    (rf"(\bОГРНИП\s*(?:№\.?|:)?\s*)({_DIGIT_VALUE}{{15,17}})", 2),
    (rf"(\bОГРН\s*(?:№\.?|:)?\s*)({_DIGIT_VALUE}{{13,15}})", 2),
    ...
    (
        ...
        r"(?:организации|предприятия|компании|юридического\s+лица|работодателя)?\s*[:—\-]\s*)"
        r"([^\n;]+)",
        2,
    ),
    (
        r"(\b(?:Организация|Предприятие|Компания|Наименование(?:\s+организации|\s+предприятия)?)\s*[:—\-]\s*)"
        r"([^\n;]+)",
        2,
    ),
    # Адрес — метка остаётся
    (
        r"((?:Юридический|Фактический|Почтовый|Место\s+нахождения)?\s*"
        r"Адрес(?:\s+организации|\s+предприятия|\s+регистрации)?\s*[:—\-]\s*)"
        r"([^\n;]+)",
        2,
    ),
    (r"(\b(?:Местонахождение|Адрес\s+места\s+нахождения)\s*[:—\-]\s*)([^\n;]+)", 2),
    # ФИО и должностные лица — метка остаётся, значение маскируется
    (r"(\bФ\.?\s*И\.?\s*О\.?\s*(?:\(полностью\))?\s*[:—\-]\s*)([^\n;]+)", 2),
    (
        r"(\bФамилия,?\s*имя,?\s*отчество\s*[:—\-]\s*)([^\n;]+)",
        2,
    ),
    (
        r"(\b(?:ФИО|Ф\.?\s*И\.?\s*О\.?)\s*(?:\(полностью\))?\s*[:—\-]\s*)([^\n;]+)",
        2,
    ),
    (
        r"(\b(?:Генеральный\s+)?(?:директор|Директор|Руководитель|"
        r"Зам\.?\s*(?:ген\.?\s*)?директора|Председатель|"
        r"Главный\s+бухгалтер|Бухгалтер|"
        r"Утвержда(?:ю|л(?:\s*\([^)]*\))?)|Согласова(?:но|л(?:\s*\([^)]*\))?)|"
        r"Подпись|Исполнитель|Составил|"
        r"С\s+документом\s+ознакомлен|"
        r"Ответственный(?:\s+лицо)?(?:\s+за\s+[^\n:;]+)?)\s*[:—\-]\s*)"
        r"([^\n;]+)",
        2,
    ),
    (
        r"(\b(?:Генеральный\s+)?(?:директор|Директор|Руководитель)\s*\n\s*)"
        r"([А-ЯЁ][а-яё\-]{1,40}(?:\s+[А-ЯЁ][а-яё\-\.]{1,40}){1,2})",
        2,
    ),
]

_FIO_WRAPPED_PATTERNS: List[Tuple[str, int]] = [
    (r"([/\(])([А-ЯЁ][а-яё\-]{1,40}\s+[А-ЯЁ]\.\s*[А-ЯЁ]\.)([/\)])", 2),
    (
        r"([/\(])([А-ЯЁ][а-яё\-]{1,40}\s+[А-ЯЁ][а-яё\-]{1,40}\s+[А-ЯЁ][а-яё\-]{1,40})([/\)])",
        2,
    ),
]

_FIO_POSITION_PATTERNS: List[Tuple[str, int]] = [
    (
        r"(\b(?:Начальник|Заместитель\s+начальника|Зам\.?\s*начальника)\s+"
        r"(?:[А-ЯЁа-яё«»\"][^\n:;]{2,50}?)\s+)"
        r"([А-ЯЁ][а-яё\-]{1,40}\s+[А-ЯЁ]\.\s*[А-ЯЁ]\.)",
        2,
    ),
]
...
_FIO_PATTERNS = [
    ...
    r"\b[А-ЯЁ]{2,40}\s+[А-ЯЁ]{2,40}\s+[А-ЯЁ]{2,40}\b",
]
...
def _looks_like_fio(text: str) -> bool:
    ...
    if any(w.lower().strip(".,;:") in _NOT_FIO_WORDS for w in words):
        return False
    return all(
        re.fullmatch(r"[А-ЯЁа-яёA-Z\-]+", w.strip(".,;:"))
        for w in words
    )
...
def mask_vnd_sensitive_data(text: str) -> str:
    """Замаскировать наименование, ИНН, ОГРН, адрес и ФИО в тексте ВНД."""
    ...
    result = _apply_patterns(result, _FIO_WRAPPED_PATTERNS)
    result = _apply_patterns(result, _FIO_POSITION_PATTERNS)
